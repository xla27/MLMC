...
def cfd_call_fine(valIns_M, valIns_T, valIns_P, valIns_Bn2, valIns_Bo2, l, i):
    baseFolder = '/global-scratch/bulk_pool/nsarman/sobol_dw5/'
    stringPart = 'level'
    ...
    if os.path.isfile(filePath): # controlla se il file esiste
        ...
        if os.path.isfile(csv_path_surf):
            ...
            p_i = (data_surf['Var18'] ).tolist() # pressure normalized w.r.t asymptotic pressure
            Ttr_i = (data_surf['Var19'] ).tolist() # rototranslational temperature normalized w.r.t asymptotic rototranslational temperature
            Tve_i = (data_surf['Var20'] ).tolist() # vibrational temperature normalized w.r.t asymptotic vibrational temperature
            M_i = (data_surf['Var23'] ).tolist() # mach normalized w.r.t asymptotic mach
